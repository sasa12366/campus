...
import org.ksu.schedule.rest.dto.ConflictScheduleResponse;
...
public class ScheduleController {
    ...
    public List<ScheduleDto> getAllSchedules() {
        ...
        if (facultyAccessService.isAdmin()) {
            schedules = facultyAccessService.filterSchedulesByAccess(schedules);
        }
    ...
    public void delete(@PathVariable int id) {
        ...
        if (facultyAccessService.isAdmin()) {
            ...
            // Проверяем доступ через факультет группы
            if (schedule.getSubgroup() != null && schedule.getSubgroup().getGroup() != null) {
                facultyAccessService.checkAccessToGroup(schedule.getSubgroup().getGroup());
            }
        }
    ...
    public ResponseEntity<?> insert(@RequestParam String parity,
        ...
        try {
            Schedule schedule = scheduleService.insert(parity, subgroup_id, subject_id, teacher_id, dayWeek, timeStart, timeEnd, classroom);
            return ResponseEntity.ok(ScheduleDto.toDto(schedule));
        } catch (ScheduleConflictException ex) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(
                    ConflictScheduleResponse.builder()
                            .message(ex.getMessage())
                            .conflict(ScheduleDto.toDto(ex.getConflict()))
                            .build()
            );
        }
    }
     ...
     * @param timeEnd время окончания.
     * @param classroom аудитория.
     * @return DTO обновленного расписания.
     */
    ...
    public ResponseEntity<?> update(@PathVariable int id,
                              ...
                              @RequestParam String classroom) {
        try {
            Schedule schedule = scheduleService.update(id, parity, subgroup_id, teacher_id, subject_id, dayWeek, timeStart, timeEnd, classroom);
            return ResponseEntity.ok(ScheduleDto.toDto(schedule));
        } catch (ScheduleConflictException ex) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(
                    ConflictScheduleResponse.builder()
