...
import org.ksu.schedule.service.impl.exception.ScheduleConflictException;
...
public class ScheduleServiceImpl implements ScheduleService {
     ...
     */
    @Override
    public Schedule insert(String parity, int subgroup_id, int teacher_id, int subject_id, String dayWeek, String timeStart, String timeEnd, String classroom) {
        Subgroup subgroup = subgroupRepository.findById(subgroup_id).orElse(null);
        ...
        ensureNoConflicts(subgroup != null ? subgroup.getId() : null, dayWeek, timeStart, timeEnd, parity, null);
     ...
     */
    @Override
    public Schedule update(int id, String parity, int subgroup_id, int teacher_id, int subject_id, String dayWeek, String timeStart, String timeEnd, String classroom) {
        Subgroup subgroup = subgroupRepository.findById(subgroup_id).orElse(null);
        Teacher teacher = teacherRepository.findById(teacher_id).orElse(null);
        Subject subject = subjectRepository.findById(subject_id).orElse(null);

        ensureNoConflicts(subgroup != null ? subgroup.getId() : null, dayWeek, timeStart, timeEnd, parity, id);
    ...
    private void ensureNoConflicts(Integer subgroupId, String dayWeek, String timeStart, String timeEnd, String parity, Integer excludeId) {
        if (subgroupId == null) {
            return;
        }
        List<Schedule> conflicts = scheduleRepository.findConflicts(subgroupId, dayWeek, timeStart, timeEnd, parity);
        if (excludeId != null) {
        ...
        if (!conflicts.isEmpty()) {
            throw new ScheduleConflictException(conflicts.get(0), "Найден конфликт с существующим занятием для подгруппы");
        }
    }
}
