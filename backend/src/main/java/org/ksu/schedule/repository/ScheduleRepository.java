...
public interface ScheduleRepository extends JpaRepository<Schedule, Integer> {
    ...
    @Query("SELECT s FROM Schedule s WHERE s.subgroup.id = :subgroupId AND s.dayWeek = :dayWeek " +
            "AND (s.parity = :parity OR s.parity = 'ВСЕГДА' OR :parity = 'ВСЕГДА') " +
            "AND s.timeStart < :timeEnd AND s.timeEnd > :timeStart")
    List<Schedule> findConflicts(int subgroupId, String dayWeek, String timeStart, String timeEnd, String parity);
}
